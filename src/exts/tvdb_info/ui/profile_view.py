    ...
    async def _initialize(self) -> None:
        ...
        for item in self.watched_list.items:
            match item.kind:
                case UserListItemKind.MOVIE:
                    refreshed_item = await refresh(self.bot.db_session, item, ["movie"])
                    watched_movies.append(refreshed_item.movie)
                case UserListItemKind.SERIES:
                    refreshed_item = await refresh(self.bot.db_session, item, ["series"])
                    watched_shows.append(refreshed_item.series)
                case UserListItemKind.EPISODE:
                    refreshed_item = await refresh(self.bot.db_session, item, ["episode"])
                    refreshed_item.episode = await refresh(self.bot.db_session, item.episode, ["series"])
                    watched_episodes.append(refreshed_item.episode)
        ...
        for item in self.favorite_list.items:
            match item.kind:
                case UserListItemKind.MOVIE:
                    refreshed_item = await refresh(self.bot.db_session, item, ["movie"])
                    favorite_movies.append(refreshed_item.movie)
                case UserListItemKind.SERIES:
                    refreshed_item = await refresh(self.bot.db_session, item, ["series"])
                    favorite_shows.append(refreshed_item.series)
                case UserListItemKind.EPISODE:
